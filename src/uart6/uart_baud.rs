...
impl From<crate::R<UART_BAUD_SPEC>> for R {
    #[inline(always)]
...
impl From<crate::W<UART_BAUD_SPEC>> for W {
    #[inline(always)]
...
impl R {
    #[doc = "Bits 0:15 - Baud Rate Divider The field indicates the baud rate divider. This filed is used in baud rate calculation. The detail description is shown in Table 6.134."]
    #[inline(always)]
    pub fn brd(&self) -> BRD_R {
        BRD_R::new((self.bits & 0xffff) as u16)
    }
    #[doc = "Bits 24:27 - Extra Divider for BAUD Rate Mode 1 This field is used for baud rate calculation in mode 1 and has no effect for baud rate calculation in mode 0 and mode 2. The detail description is shown in Table 6.134."]
    #[inline(always)]
    pub fn edivm1(&self) -> EDIVM1_R {
        EDIVM1_R::new(((self.bits >> 24) & 0x0f) as u8)
    }
    #[doc = "Bit 28 - BAUD Rate Mode Selection Bit 0 This bit is baud rate mode selection bit 0. UART provides three baud rate calculation modes. This bit combines with BAUDM1 (UART_BAUD\\[29\\]) to select baud rate calculation mode. The detail description is shown in Table 6.134."]
    #[inline(always)]
    pub fn baudm0(&self) -> BAUDM0_R {
        BAUDM0_R::new(((self.bits >> 28) & 0x01) != 0)
    }
    #[doc = "Bit 29 - BAUD Rate Mode Selection Bit 1 This bit is baud rate mode selection bit 1. UART provides three baud rate calculation modes. This bit combines with BAUDM0 (UART_BAUD\\[28\\]) to select baud rate calculation mode. The detail description is shown in Table 6.134. Note: In IrDA mode must be operated in mode 0."]
    #[inline(always)]
