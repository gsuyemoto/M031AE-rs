...
impl From<crate::R<UART_MODEM_SPEC>> for R {
    #[inline(always)]
...
impl From<crate::W<UART_MODEM_SPEC>> for W {
    #[inline(always)]
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum RTSACTLV_A {
    #[doc = "0: nRTS pin output is high level active"]
    _0 = 0,
...
impl<'a> RTSACTLV_W<'a> {
    ...
    #[doc = "nRTS pin output is high level active"]
    #[inline(always)]
    pub fn _0(self) -> &'a mut W {
...
impl R {
    #[doc = "Bit 1 - nRTS (Request-to-send) Signal Control This bit is direct control internal nRTS signal active or not, and then drive the nRTS pin output with RTSACTLV bit configuration. Note 1: The nRTS signal control bit is not effective when nRTS auto-flow control is enabled in UART function mode. Note 2: The nRTS signal control bit is not effective when RS-485 auto direction mode (AUD) is enabled in RS-485 function mode. Note 3: Single-wire mode is support this feature."]
    #[inline(always)]
    pub fn rts(&self) -> RTS_R {
        RTS_R::new(((self.bits >> 1) & 0x01) != 0)
    }
    #[doc = "Bit 9 - nRTS Pin Active Level This bit defines the active level state of nRTS pin output. Note 1: Refer to Figure 6.1313 and Figure 6.1314 for UART function mode. Note 2: Refer to Figure 6.1317 and Figure 6.1318 for RS-485 function mode. Note 3: Before setting this bit, TXRXDIS (UART_FUNCSEL\\[3\\]) should be set then waited for TXRXACT (UART_FIFOSTS\\[31\\]) is cleared. When the configuration is done, cleared TXRXDIS (UART_FUNCSEL\\[3\\]) to activate UART controller."]
    #[inline(always)]
    pub fn rtsactlv(&self) -> RTSACTLV_R {
        RTSACTLV_R::new(((self.bits >> 9) & 0x01) != 0)
    }
    #[doc = "Bit 13 - nRTS Pin Status (Read Only) This bit mirror from nRTS pin output of voltage logic status."]
    #[inline(always)]
