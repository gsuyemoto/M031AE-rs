...
impl From<crate::R<UART_IRDA_SPEC>> for R {
    #[inline(always)]
...
impl From<crate::W<UART_IRDA_SPEC>> for W {
    #[inline(always)]
...
impl R {
    ...
    #[doc = "Bit 5 - IrDA Inverse Transmitting Output Signal Note 1: Before setting this bit, TXRXDIS (UART_FUNCSEL\\[3\\]) should be set then waited for TXRXACT (UART_FIFOSTS\\[31\\]) is cleared. When the configuration is done, cleared TXRXDIS (UART_FUNCSEL\\[3\\]) to activate UART controller. Note 2: This bit is valid when FUNCSEL (UART_FUNCSEL\\[1:0\\]) is select IrDA function."]
    #[inline(always)]
    pub fn txinv(&self) -> TXINV_R {
        TXINV_R::new(((self.bits >> 5) & 0x01) != 0)
    }
    #[doc = "Bit 6 - IrDA Inverse Receive Input Signal Note 1: Before setting this bit, TXRXDIS (UART_FUNCSEL\\[3\\]) should be set then waited for TXRXACT (UART_FIFOSTS\\[31\\]) is cleared. When the configuration is done, cleared TXRXDIS (UART_FUNCSEL\\[3\\]) to activate UART controller. Note 2: This bit is valid when FUNCSEL (UART_FUNCSEL\\[1:0\\]) is select IrDA function."]
    #[inline(always)]
